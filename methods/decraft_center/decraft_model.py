...
class TransEmbedding(nn.Module):
    ...
    def __init__(self, df=None, device='cpu', dropout=0.2, in_feats=82, cat_features=None):
        ...
        :param df: the feature
                ...
                :param cat_feature: category features 指的是card、transaction、merchant 账户类型
        """
        ...
        self.label_table = nn.Embedding(3, in_feats, padding_idx=2).to(device)  # 没有使用的哦
        self.time_emb = None
        self.emb_dict = None
        self.label_emb = None
...
class GraphAttnModel(nn.Module):
    ...
    # train_batch_logits = model(blocks, batch_inputs(blocks[0].srcnodes()中的标签), lpa_labels(指的是blocks[0].srcnodes()中的标签，其中output_nodes中的标签都是2), batch_work_inputs（指的账户类别的特征）)
